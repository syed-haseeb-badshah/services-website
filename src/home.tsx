...
export default function Home(){return <div className="studio-home">
 ...
 <section className="studio-stats" aria-label="Our offering at a glance"><div className="wrap"><dl className="stats-strip">{[[400,'Total clients'],[services.length,'Services offered'],[serviceCategories.length,'Core disciplines'],[projects.length,'Concept projects'],[packages.length,'Package options'],[4,'Process stages']].map(([value,label])=><div className="stat-item" key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl></div></section>
 <ProjectShowcase/>
 <HomeServices/>
 ...
 <section className="new-cta"><div className="wrap"><p className="eyebrow">LET’S MAKE YOUR NEXT MOVE COUNT</p><h2>Ready for a better<br/><em>first impression?</em></h2><div><p>Tell us where you are.<br/>Let’s figure out what comes next.</p><Link className="button" to="/contact">Start a conversation <span aria-hidden="true">↗</span></Link></div></div></section>
 </div></div>}
