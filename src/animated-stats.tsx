...
export default function AnimatedStats({ items }: { items: Statistic[] }) {
  ...
  useEffect(() => {
    ...
    if (preference.matches) finish();
    else if (strip.current) observer.observe(strip.current);
  ...
  return (
    <dl ref={strip} className="stats-strip animated-stats">
      {items.map(([value, label]) => {
        ...
        return (
          <div className="stat-item" key={label}>
            <dt>{label}</dt>
            <dd>
              <span className="stat-value-for-readers">{value}</span>
              <span aria-hidden="true">
                {Math.floor(numeric * progress).toLocaleString("en-GB")}
                {suffix}
              </span>
            </dd>
          </div>
