...
export type ShowcaseItem={title:string;poster:string;alt:string;video?:string;category:string;description:string;href:string;linkLabel:string};
const defaultItems:ShowcaseItem[]=showcaseMedia.map((media,index)=>({...media,category:serviceCategories[index].title,description:serviceCategories[index].description,href:'/services?category='+serviceCategories[index].slug,linkLabel:'Explore '+['websites','photo & video','brand & content','technology'][index]}));

function ServiceCard({media,index,duplicate=false}:{media:ShowcaseItem;index:number;duplicate?:boolean}){
 ...
 return <article ref={card} className="proud-card" aria-hidden={duplicate||undefined} onPointerEnter={event=>{if(event.pointerType==='mouse')start()}} onPointerLeave={stop} onPointerCancel={stop}>
  ...
  <div className="proud-card-copy"><p className="proud-category">{media.category}</p><h3>{media.title}</h3><p className="proud-description">{media.description}</p><Link tabIndex={duplicate?-1:undefined} to={media.href}>{media.linkLabel} <span aria-hidden="true">↗</span></Link></div>
 </article>
}

export default function ProjectShowcase({items=defaultItems,title='Projects we’re proud of',description='Four connected disciplines. Explore our creative direction across websites, visual storytelling, branding, and technology.',note='Illustrative concept previews across our four main services.'}:{items?:ShowcaseItem[];title?:string;description?:string;note?:string}){
 ...
 const normalizeLoop=()=>{const el=track.current;const cycle=stepSize()*items.length;if(el&&cycle&&el.scrollLeft>=cycle-.5)el.scrollTo({left:Math.max(0,el.scrollLeft-cycle),behavior:'instant'})};
 ...
 return <section className="proud-section section" aria-labelledby="proud-title"><div className="wrap">
  <header className="proud-heading"><h2 id="proud-title">{title}</h2><p>{description}</p></header>
  <div className="proud-carousel" onPointerEnter={()=>setHovering(true)} onPointerLeave={()=>setHovering(false)} onPointerCancel={()=>setHovering(false)} onFocusCapture={()=>setFocused(true)} onBlurCapture={event=>{if(!event.currentTarget.contains(event.relatedTarget))setFocused(false)}}><button className="proud-arrow proud-prev" type="button" aria-label="Previous service card" aria-controls="proud-track" onClick={()=>move(-1)}>‹</button><div ref={track} id="proud-track" className="proud-track" onScroll={normalizeLoop} tabIndex={0} role="region" aria-label="Service project previews">{[0,1].flatMap(copy=>items.map((media,i)=><ServiceCard key={copy+'-'+i} media={media} index={i} duplicate={copy===1}/>))}</div><button className="proud-arrow proud-next" type="button" aria-label="Next service card" aria-controls="proud-track" onClick={()=>move(1)}>›</button></div>
  <p className="proud-note">{note}</p><div className="proud-playback"><button type="button" onClick={()=>setPaused(value=>!value)} aria-pressed={paused} aria-controls="proud-track">{paused?'Resume slideshow':'Pause slideshow'}</button></div>
 </div></section>
}
